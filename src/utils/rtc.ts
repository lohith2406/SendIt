...
export class PeerTransfer {
  ...
  private setupDataChannel() {
    ...
    this.dataChannel.onmessage = (e) => {
      const msg = JSON.parse(e.data) as
        | { messageType: 'file-info'; meta: FileMeta }
        | { messageType: 'file-chunk'; fileId: string; chunkNumber: number; data: string }
        | { messageType: 'file-complete'; fileId: string };
  ...
  private sendSignal(msg: Record<string, unknown>, attempt = 0) {
    ...
    if (this.ws.readyState === WebSocket.CONNECTING && attempt < MAX_RETRIES) {
      console.warn('🔁 Retrying sendSignal (attempt ' + attempt + ')...');
